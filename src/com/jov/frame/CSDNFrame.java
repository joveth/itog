...
public class CSDNFrame extends Fragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.csdn_frame, container, false);
	...
	private void runThread() {
		if (!Common.isNetworkConnected(context)) {
			if (list.size() == 0) {
				List<BlogBean> result = db.getBlog(Constants.CSDN_FLAG_2);
				list.addAll(result);
			}
	...
	private Handler csdnHand = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 110:
				break;
			default:
				Toast.makeText(context, "【CSDN】请求出现异常，您可以尝试再次刷新！",
	...
	public void onMore() {
		if (!isDoingUpdate) {
			...
			ThreadPoolUtils.execute(new HTMLParser(nextPageCSDNHand,
					Constants.CSDN_BLOG + "?&page=" + pageNo, false));
			isDoingUpdate = true;
		}
	}
}
